...
contract LendingAuction {
    ...
    function startAuction(
        ...
        IERC20 loanCurrency,
        uint256 loanAmount,
        uint256 maxLoanRepaymentAmount,
        uint256 auctionDepositAmount,
        uint256 minDecrementFactorNumerator,
        uint256 auctionEndTIme,
        uint256 loanRepaymentDeadline
    ) external returns (uint256 loanAuctionId) {
        ...
        require(
            loanRepaymentDeadline > auctionEndTIme,
            "CANT REPAY BEOFRE AUCTION END"
        );
        require(auctionDepositAmount > 0, "MUST DEPOSIT");
        require(
            auctionDepositAmount <= loanAmount,
            "DEPOSIT CANNOT EXCEED LOAN"
        );
        require(
            loanAmount < maxLoanRepaymentAmount,
            "REPAYMENT LESS THAN LOAN"
        );
        ...
        IERC721(collateralAddress).transferFrom(
            msg.sender,
            address(this),
            collateralId
        );
    ...
    function cancelAuction(uint256 loanAuctionId) external {
        ...
        require(
            loanAuctions[loanAuctionId].topLender == address(0),
            "CANNOT CANCEL ACTIVE AUCTION"
        );
        IERC721(loanAuctions[loanAuctionId].collateralAddress).transferFrom(
            address(this),
            msg.sender,
            loanAuctions[loanAuctionId].collateralId
        );
    ...
    function bid(uint256 loanAuctionId, uint256 repayAmount) external {
        require(
            msg.sender != loanAuctions[loanAuctionId].borrower,
            "CANNOT BID ON YOUR AUCTION"
        );
        require(
            loanAuctions[loanAuctionId].loanAmount < repayAmount,
            "REPAYMENT LESS THAN LOAN"
        );
        require(
            repayAmount <= loanAuctions[loanAuctionId].loanRepaymentAmount,
            "MUST BID WITHIN LIMIT"
        );
        require(
            loanAuctions[loanAuctionId].topLender != address(0) ||
                repayAmount <
                loanAuctions[loanAuctionId].loanAmount +
                    ((loanAuctions[loanAuctionId].loanRepaymentAmount -
                        loanAuctions[loanAuctionId].loanAmount) *
                        loanAuctions[loanAuctionId]
                            .minDecrementFactorNumerator) /
                    DENOMINATOR,
            "MUST BID BETTER"
        );

        if (loanAuctions[loanAuctionId].topLender != address(0))
            IERC20(loanAuctions[loanAuctionId].loanCurrency).transferFrom(
                msg.sender,
                loanAuctions[loanAuctionId].topLender,
                loanAuctions[loanAuctionId].auctionDepositAmount
            );
        else
            IERC20(loanAuctions[loanAuctionId].loanCurrency).transferFrom(
                msg.sender,
    ...
    function getLoan(uint256 loanAuctionId) external returns (bool success) {
        ...
        require(
            block.timestamp > loanAuctions[loanAuctionId].auctionEndTime,
            "AUCTION NOT COMPLETED"
        );
        require(
            loanAuctions[loanAuctionId].auctionDepositAmount > 0,
            "LOAN ALREADY TAKEN"
        );
        require(
            IERC721(loanAuctions[loanAuctionId].collateralAddress).ownerOf(
                loanAuctions[loanAuctionId].collateralId
            ) == address(this),
            "MISSING COLLATERAL"
        );
        require(
            block.timestamp < loanAuctions[loanAuctionId].loanRepaymentDeadline,
            "PAST REPAYMENT DEADLINE"
        );
        if (
            IERC20(loanAuctions[loanAuctionId].loanCurrency).allowance(
                loanAuctions[loanAuctionId].topLender,
                address(this)
            ) >=
            loanAuctions[loanAuctionId].loanAmount -
            ...
            IERC721(loanAuctions[loanAuctionId].collateralAddress).transferFrom(
                    address(this),
                    loanAuctions[loanAuctionId].borrower,
                    loanAuctions[loanAuctionId].collateralId
                );
            success = false;
        }
    ...
    function repayLoan(uint256 loanAuctionId) external {
        ...
        require(
            block.timestamp <=
                loanAuctions[loanAuctionId].loanRepaymentDeadline,
            "PAST REPAYMENT DEADLINE"
        );
        IERC20(loanAuctions[loanAuctionId].loanCurrency).transferFrom(
            msg.sender,
    ...
    function liquidateLoan(uint256 loanAuctionId) external {
        ...
        require(
            block.timestamp > loanAuctions[loanAuctionId].loanRepaymentDeadline,
            "REPAYMENT DEADLINE NOT REACHED"
        );
        if (loanAuctions[loanAuctionId].auctionDepositAmount > 0) {
            // Loan not taken
